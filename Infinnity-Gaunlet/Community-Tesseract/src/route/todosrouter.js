...
TodosRouter.get("/to-dos", async function (req, res) {
  try {
    ...
    res
      .status(500)
      .send({ messagge: "Something went wrong trying, to get to dos", error });
  }
});
...
TodosRouter.post("/to-do", validator, async function (req, res) {
  try {
    ...
    res.status(500).send({
      messagge: "Something went wrong, trying to create a to do",
      error,
    });
  }
});
...
TodosRouter.delete("/to-do/:id", async function (req, res) {
  try {
    ...
    res
      .status(500)
      .send(
        { messagge: "Something went wrong trying yo delete a to-do" },
        error.messagge
      );
  }
});
...
TodosRouter.patch("/to-do/:id", async function (req, res) {
  try {
    ...
    res
      .status(500)
      .send(
        { messagge: "Something went wrong trying to update  a to do" },
        error.message
      );
  }
});

TodosRouter.patch("/to-do/comp/:id", async function (req, res) {
  try {
    const db = await getDbConnection();
    const id = req.params.id;
    const todoExists = await db.get(`SELECT * FROM todos WHERE id = ?`, id);
    ...
    res.send({ message: "To-do Completed " });
    await db.close();
  } catch (error) {
    res
      .status(500)
      .send(
        { messagge: "Something went wrong trying to complete a to-do" },
        error.message
      );
  }
});
